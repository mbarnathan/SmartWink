/**
 *  Lutron Plug-in Dimmer device type for SmartWink.
 *
...
metadata {
    definition(
            ...
            description: "Lutron Caseta Wireless Plug-in Dimmer",
...
def handleEvent(parsedEvent, hub, json) {
    log.info "Plug-in Dimmer ${device.deviceNetworkId} handling event: ${json}"
    if (json.containsKey("value")) {
        ...
        log.warn "Plug-in dimmer level event ${json} has no value attribute."
    }
    return null
}
...
def parse(description) {
    ...
    if (keyVal[0] in ["level", "switch"]) {
        ...
        log.trace "Plug-in dimmer was updated"
        return null
    } else {
        log.warn "Unknown event in plug-in dimmer parse(): ${description}"
        return null
    }
}
